...
export class BounceUpComponent implements OnInit {
  ...
  public ngOnInit(): void {
    const highScoresCollection = collection(this.firestore, 'high-scores')
    const scoresObservable = collectionData(highScoresCollection);
  ...
  //   this.canvas.addEventListener('mouseover' && 'click', (e: any) => {
  //     if (!this.started) {
  ...
  //   this.canvas.addEventListener('keydown', (key: any) => {
  ...
  //   this.canvas.addEventListener('keyup', (key: any) => {
  //     if (key.key === 'ArrowLeft' || key.key === 'a') {
  //       this.goLeft = false;
  //     }
  //     else if (key.key === 'ArrowRight' || key.key === 'd') {
  ...
  //     if (this.bouncer.x + this.bouncer.w <= this.canvas.width) {
  //       this.bouncer.x += this.bouncer.vx;
  //     }
  //   }
  //   this.ctx.fillStyle = 'black'
